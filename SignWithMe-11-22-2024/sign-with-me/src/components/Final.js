...
const Final = () => {
  ...
  const handleFinish = async (finalScore) => {
    ...
    const progressScore = finalScore / questions.length;

    if (userName) {
      try {
        // Step 1: Fetch the current exam data from the server
        const response = await axios.get(
          "http://localhost:5000/api/exams/get-exam",
          {
            params: {
              examId: "final",
              userName: userName,
        ...
        // Step 2: Compare the progress scores
        if (exam && exam.progress >= progressScore) {
          console.log(
            "Existing progress is higher or equal. No update needed."
          );
        } else {
          // Step 3: Update the exam only if the new score is higher
          await axios.put("http://localhost:5000/api/exams/update-exam", {
            examId: "final",
            userName: userName,
            progress: progressScore,
          });

          console.log("Exam progress updated successfully!");
        }
      } catch (error) {
        console.error("Error handling exam update:", error);
      }
    } else {
      console.error("User is not logged in, unable to update exam.");
    }
  };

  const checkAnswer = (answer) => {
    const isCorrect = answer === loadQuestion().correct;
    ...
    setScore((prevScore) => {
      ...
      if (currentQuestionIndex + 1 >= questions.length) {
        handleFinish(newScore); // Pass the correct final score
        setShowResult(true);
      }
